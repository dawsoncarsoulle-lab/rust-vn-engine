...
fn visit(path:&Path)->Result<(),Box<dyn std::error::Error>> {
    for entry in fs::read_dir(path)? {
        ...
        if entry.file_type()?.is_dir(){visit(&path)?;}
            ...
            graph.materialize_visible_defaults().map_err(|e|e.to_string())?;
...
fn main()->Result<(),Box<dyn std::error::Error>> {
    ...
    if args.len()!=2||args[0]!="--apply"{return Err("Usage: relayout_import --apply GRAPH_DIRECTORY (changes positions only)".into());}
