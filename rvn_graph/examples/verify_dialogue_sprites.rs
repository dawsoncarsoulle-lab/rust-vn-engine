...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    fs::create_dir(&dir)?;
    let mut g = GraphDocument::new(GraphId::new(901), GraphKind::Label { name: "start".into() });
    g.characters.insert("mara".into(), "Mara".into());
    let mut previous = g.add_catalog_node(NodeKind::Label, [0.0, 0.0])?;
    for (i, (path, text)) in [
        (Some("sprites/mara/neutral.png"), "Image branchee : Mara doit etre visible."),
        (None, "Pin vide : Mara reste visible, sans changer de sprite."),
        (None, "Apres Retirer le sprite : Mara parle hors champ."),
        (Some("sprites/mara/determinee.png"), "Nouvelle image branchee : Mara reapparait."),
    ].into_iter().enumerate() {
        let x = 350.0 + i as f64 * 650.0;
        if i==2 {
            for offset in 0..2 {
                let hide=g.add_catalog_node(NodeKind::SpriteHide,[x-300.0+offset as f64*180.0,-220.0])?;
