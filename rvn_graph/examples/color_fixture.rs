...
fn main() {
    let path = std::env::args()
        .nth(1)
        .expect("isolated fixture output path");
    ...
    let color = g
        .add_catalog_node(NodeKind::MakeColor, [0.0, 100.0])
        .unwrap();
    let red = g.pin_by_key(color, "r").unwrap().id;
    g.pins.get_mut(&red).unwrap().default_value = Some(PropertyValue::Float(255.0));
    for (i, (name, kind, value)) in [
        ("Exemple Num", ValueType::Int, PropertyValue::Int(0)),
    ...
    .enumerate()
    {
        ...
        let set = g
            .add_catalog_node(
                ...
                [240.0 + (i % 2) as f64 * 280.0, (i / 2) as f64 * 150.0],
            )
            .unwrap();
        let get = g
            .add_catalog_node(
                ...
                [
                    285.0 + (i % 2) as f64 * 280.0,
                    95.0 + (i / 2) as f64 * 150.0,
                ],
            )
            .unwrap();
        for node in [set, get] {
            g.set_property(node, "name", PropertyValue::String(name.clone()))
                .unwrap();
        }
