...
fn make_color_exports_dynamic_rgba_and_preserves_inline_inputs() {
    ...
    g.set_property(effect, "apply_tint", PropertyValue::Bool(true)).unwrap();
    default(&mut g, effect, "character", PropertyValue::String("alice".into()));
    ...
    assert!(source.contains("make_color_rgb(255.0, 0.0, 0.0, 0.5)"), "{source}");
    ...
    let rvn_parser::Statement::SetVar { value, .. } = &script[0] else { panic!("assignment expected") };
    assert_eq!(rvn_core::eval_expr(value, &Default::default()).unwrap(), rvn_parser::Value::Str("#ff800080".into()));
    let rgb = rvn_parser::parse("set c = make_color_rgb(255, 128, 0, 0.5)\n").unwrap();
    let rvn_parser::Statement::SetVar { value, .. } = &rgb[0] else { panic!() };
    assert_eq!(rvn_core::eval_expr(value, &Default::default()).unwrap(), rvn_parser::Value::Str("#ff800080".into()));
}
...
fn legacy_colors_upgrade_once_without_changing_sources_or_alpha() {
    let (mut g, _) = graph();
    let color = node(&mut g, NodeKind::MakeColor);
    g.nodes.get_mut(&color).unwrap().properties.remove("rgb_max");
    ...
    g.set_property(literal, "value", PropertyValue::Float(0.75)).unwrap();
    connect(&mut g, literal, "value", color, "g");
    assert_eq!(g.upgrade_color_range().unwrap(), 1);
    assert_eq!(g.pin_by_key(color, "r").unwrap().default_value, Some(PropertyValue::Float(127.5)));
    assert_eq!(g.pin_by_key(color, "a").unwrap().default_value, Some(PropertyValue::Float(0.25)));
    assert_eq!(g.nodes[&literal].properties["value"], PropertyValue::Float(0.75));
    let scale = g.nodes.values().find(|n| n.kind == NodeKind::MathMultiply).unwrap();
    assert_eq!(g.pin_by_key(scale.id, "right").unwrap().default_value, Some(PropertyValue::Float(255.0)));
...
fn set_inline_values_remain_inline_after_save_and_load() {
    let (mut g, _) = graph();
    for (name, kind, value) in [("flag", ValueType::Bool, PropertyValue::Bool(false)),
        ("score", ValueType::Int, PropertyValue::Int(0)),
        ("text", ValueType::String, PropertyValue::String("texte ici".into()))] {
        g.add_variable(name, kind, value.clone()).unwrap();
        let set = node(&mut g, NodeKind::SetVariable);
        g.set_property(set, "name", PropertyValue::String(name.into())).unwrap();
...
fn graph() -> (GraphDocument, NodeId) {
    let mut graph = GraphDocument::new(GraphId::new(1), GraphKind::Label { name: "start".into() });
    let root = graph.add_catalog_node(NodeKind::Label, [0.0, 0.0]).unwrap();
    (graph, root)
}
fn node(g: &mut GraphDocument, kind: NodeKind) -> NodeId { g.add_catalog_node(kind, [0.0, 0.0]).unwrap() }
fn connect(g: &mut GraphDocument, a: NodeId, ak: &str, b: NodeId, bk: &str) {
    g.connect(g.pin_by_key(a, ak).unwrap().id, g.pin_by_key(b, bk).unwrap().id).unwrap();
}
...
fn named_label_can_be_reentered_with_jump() {
    ...
    g.set_property(label, "label", PropertyValue::String("door".into())).unwrap();
    ...
    g.set_property(label, "label", PropertyValue::String("start".into())).unwrap();
    assert!(transpile(&g).is_err());
}
#[test]
fn typed_function_arguments_and_mixed_lists_preserve_types() {
    let (mut g, root) = graph();
    g.add_variable("values", ValueType::List(Box::new(ValueType::Any)), PropertyValue::StringList(vec![])).unwrap();
    let set = node(&mut g, NodeKind::SetVariable);
    g.set_property(set, "name", PropertyValue::String("values".into())).unwrap();
    let array = node(&mut g, NodeKind::ListLiteral);
    g.resize_value_inputs(array, 3).unwrap();
    default(&mut g, array, "item_1", PropertyValue::String("hello".into()));
    default(&mut g, array, "item_2", PropertyValue::Bool(true));
    let function = node(&mut g, NodeKind::FunctionCall);
    g.set_property(function, "function", PropertyValue::String("random".into())).unwrap();
    ...
    assert!(transpile(&g).unwrap().source.contains("[random(1, 6), \"hello\", true]"));
}
#[test]
fn upgrading_legacy_list_never_reinterprets_strings() {
    let (mut g, _) = graph();
    let list = node(&mut g, NodeKind::ListLiteral);
    g.set_property(list, "items", PropertyValue::StringList(vec!["1".into(), "true".into()])).unwrap();
    g.resize_value_inputs(list, 3).unwrap();
    assert_eq!(g.pin_by_key(list, "item_0").unwrap().default_value, Some(PropertyValue::String("1".into())));
    assert_eq!(g.pin_by_key(list, "item_1").unwrap().default_value, Some(PropertyValue::String("true".into())));
...
fn choice_conditions_are_evaluated_before_display() {
    ...
    g.set_property(choice, "option_1_condition", PropertyValue::String("possede_cle".into())).unwrap();
    g.set_property(choice, "option_2_condition", PropertyValue::String("confiance >= 50".into())).unwrap();
...
fn effect_modifies_only_checked_properties() {
    ...
    g.set_property(effect, "apply_rotation", PropertyValue::Bool(true)).unwrap();
    connect(&mut g, root, "exec_out", effect, "exec_in");
    assert!(transpile(&g).unwrap().source.contains("alice.effect(rotation: 45)"));
}
#[test]
fn imagemap_supports_independent_hover_rectangle() {
    let (mut g, root) = graph();
    let map = node(&mut g, NodeKind::Imagemap);
    default(&mut g, map, "background", PropertyValue::String("room.png".into()));
    g.set_imagemap_hotspots(map, vec!["door:0:0:100:200:100:0:200:200".into()]).unwrap();
...
fn project_export_checks_destinations_and_duplicate_labels() {
    ...
    assert!(compiled.source.find("label start").unwrap() < compiled.source.find("label door").unwrap());
...
fn parser_keeps_first_list_element_and_allows_index_after_call() {
    ...
    assert!(matches!(&parsed[0], rvn_parser::Statement::SetVar { value: rvn_parser::Expr::ListLit(items), .. } if items.len() == 3));
    assert!(matches!(&parsed[1], rvn_parser::Statement::SetVar { value: rvn_parser::Expr::Index { .. }, .. }));
}
...
fn animation_typed_parameters_override_legacy_values_once() {
    ...
    default(&mut g, animation, "character", PropertyValue::String("alice".into()));
    default(&mut g, animation, "animation", PropertyValue::String("shake".into()));
    g.set_property(animation, "params", PropertyValue::StringList(vec!["duration=0.3".into()])).unwrap();
    g.set_property(animation, "animation_param_duration", PropertyValue::Float(0.7)).unwrap();
    g.set_property(animation, "animation_param_loop", PropertyValue::Bool(true)).unwrap();
...
fn exported_choice_failure_dialogue_returns_to_same_menu_in_engine() {
    ...
    g.set_property(door, "label", PropertyValue::String("door".into())).unwrap();
    let first_jump = node(&mut g, NodeKind::Jump);
    default(&mut g, first_jump, "target", PropertyValue::String("door".into()));
    ...
    g.set_property(text, "value", PropertyValue::String("Je n’ai pas la clé.".into())).unwrap();
    connect(&mut g, text, "value", dialogue, "text");
    connect(&mut g, condition, "else", dialogue, "exec_in");
    let retry = node(&mut g, NodeKind::Jump);
    default(&mut g, retry, "target", PropertyValue::String("door".into()));
    ...
    assert!(matches!(engine.step_until_interaction().unwrap(), Some(Interaction::Choice { options }) if options.len() == 2));
    engine.submit_choice(0).unwrap();
    assert!(matches!(engine.step_until_interaction().unwrap(), Some(Interaction::Dialogue { text, .. }) if text == "Je n’ai pas la clé."));
    engine.advance_dialogue().unwrap();
    assert!(matches!(engine.step_until_interaction().unwrap(), Some(Interaction::Choice { options }) if options[0] == "Ouvrir la porte"));
}
