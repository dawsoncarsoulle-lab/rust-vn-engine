...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for e in &mut page.elements {
        if e.binding.as_deref() == Some("dialogue.text") {
            e.rect = [120.0, 830.0, 1640.0, 140.0];
            if std::env::args().any(|a| a == "--styled-scroll") {
                e.appearance.scrollbar = ScrollbarStyle {
                    track: [0.0, 0.12, 0.2, 1.0],
                    thumb: [1.0, 0.5, 0.0, 1.0],
                    hover: [0.0, 1.0, 1.0, 1.0],
                    pressed: [1.0, 0.0, 1.0, 1.0],
                    width: 30.0,
                    radius: 0.0,
                };
            }
