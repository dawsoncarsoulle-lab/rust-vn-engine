...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (i, (condition, text)) in [("save.empty", "EMPLACEMENT VIDE"), ("save.filled", "SAUVEGARDE PRESENTE"), ("save.locked", "PROTEGEE"), ("save.unlocked", "NON PROTEGEE")].into_iter().enumerate() {
    ...
    for child in &mut filled_only.children { child.text = format!("FILTRE {}", child.text); }
    ...
    doc.pages.iter_mut().find(|p| p.id == "load").unwrap().elements = elements;
