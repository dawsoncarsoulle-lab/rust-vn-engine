...
fn copy_assets(source:&Path,target:&Path)->std::io::Result<()> {
    ...
    for entry in fs::read_dir(source)?{let entry=entry?;let kind=entry.file_type()?;
        if kind.is_dir(){copy_assets(&entry.path(),&target.join(entry.file_name()))?;}
        else if kind.is_file(){fs::copy(entry.path(),target.join(entry.file_name()))?;}
        else{return Err(std::io::Error::other("Unsupported asset file type"));}
    }Ok(())
}
fn main()->Result<(),Box<dyn std::error::Error>> {
    let args:Vec<_>=std::env::args().collect();
    let name=args.get(1).ok_or("Design required")?;
    if !["Sobre","Illustré","Science-fiction"].contains(&name.as_str()){return Err("Unknown design".into());}
    ...
    let source=Path::new(env!("CARGO_MANIFEST_DIR")).join("../examples/menu-designs").join(name);
    let document=fs::read(source.join("menus.rvnui"))?;
    rvn_ui::Document::from_json(std::str::from_utf8(&document)?)?.validate()?;
    fs::create_dir(root)?;copy_assets(&source.join("assets"),&root.join("assets"))?;
