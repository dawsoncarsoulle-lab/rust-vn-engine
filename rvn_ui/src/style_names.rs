...
impl Document {
    pub fn capture_style(&mut self,page:usize,element:&str,name:&str)->Result<String,String> {
        ...
        source.style=Some(name.clone());source.overrides=Default::default();
        Ok(name)
    }
    pub fn rename_style(&mut self,old:&str,new:&str)->Result<(),String> {
        ...
        for e in self.components.values_mut(){remap(std::slice::from_mut(e),old,&new);}
        Ok(())
    }
}
#[cfg(test)]mod tests {
    use super::*;
    #[test]fn rename_updates_pages_components_and_preserves_local_exceptions(){
        let mut d=Document::defaults();let id=d.pages[0].elements[0].id.clone();
        d.capture_style(0,&id,"Titres clairs").unwrap();
        let mut e=d.pages[0].elements[0].clone();e.overrides.foreground=Some([1.0,0.0,0.0,1.0]);
        ...
        assert_eq!(d.pages[0].elements[0].style.as_deref(),Some("Titres — papier"));
