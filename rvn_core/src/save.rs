...
impl SaveManager {
    fn protected_slots(&self) -> Result<std::collections::BTreeSet<u32>, SaveError> {
        ...
        #[cfg(target_arch = "wasm32")]
        let json = Self::local_storage()?
            .get_item(&self.storage_key("protected_slots"))
            .map_err(|_| {
                SaveError::Io(std::io::Error::other("lecture des protections impossible"))
            })?;
        json.map(|json| serde_json::from_str(&json).map_err(SaveError::Json))
            .unwrap_or_else(|| Ok(Default::default()))
    }
    pub fn is_protected(&self, slot: u32) -> Result<bool, SaveError> {
        self.check_slot(slot)?;
        Ok(self.protected_slots()?.contains(&slot))
    }
    pub fn set_protected(&self, slot: u32, protected: bool) -> Result<(), SaveError> {
        self.check_slot(slot)?;
        let mut slots = self.protected_slots()?;
        if protected {
            slots.insert(slot);
        } else {
            slots.remove(&slot);
        ...
        #[cfg(target_arch = "wasm32")]
        Self::local_storage()?
            .set_item(&self.storage_key("protected_slots"), &json)
            .map_err(|_| {
                SaveError::Io(std::io::Error::other("écriture des protections impossible"))
            })?;
        Ok(())
    }
    fn check_writable(&self, slot: u32) -> Result<(), SaveError> {
        if self.is_protected(slot)? {
            return Err(SaveError::SlotProtege(slot));
        }
        Ok(())
    }
    #[cfg(not(target_arch = "wasm32"))]
    ...
    pub fn set_resume_thumbnail(
        ...
        let path = match slot {
            ResumeSlot::Auto => self.autosave_path(),
            ResumeSlot::Quick => self.quicksave_path(),
        };
        let mut data = self.load_from_path(path.clone())?;
        if serde_json::to_value(&data)? != serde_json::to_value(expected)? {
            return Ok(false);
        }
        data.thumbnail = Some(thumbnail);
        self.write_atomic(&path, &serde_json::to_string_pretty(&data)?)?;
        Ok(true)
    }
...
mod tests {
    ...
    fn delayed_resume_thumbnail_never_overwrites_a_newer_save() {
        let directory =
            std::env::temp_dir().join(format!("rvn_resume_thumbnail_{}", std::process::id()));
        let manager = SaveManager::new(&directory, 10).unwrap();
        let mut state = sample_state();
        for slot in [ResumeSlot::Auto, ResumeSlot::Quick] {
            let write = |state: &GameState| match slot {
                ResumeSlot::Auto => manager.save_autosave(state, "Auto".into(), "test.rvn".into()),
                ResumeSlot::Quick => {
                    manager.save_quicksave(state, "Quick".into(), "test.rvn".into())
                }
            };
            let read = || match slot {
                ResumeSlot::Auto => manager.load_autosave(),
                ResumeSlot::Quick => manager.load_quicksave(),
            };
            write(&state).unwrap();
            let expected = read().unwrap();
            let before = serde_json::to_value(&expected).unwrap();
            assert!(manager
                .set_resume_thumbnail(slot, &expected, "@save/test.png".into())
                .unwrap());
            let mut after = serde_json::to_value(read().unwrap()).unwrap();
            after["thumbnail"] = before["thumbnail"].clone();
            assert_eq!(before, after);
            state.pc += 1;
            state.current_interactive_pc += 1;
            write(&state).unwrap();
            let newer = serde_json::to_value(read().unwrap()).unwrap();
            assert!(!manager
                .set_resume_thumbnail(slot, &expected, "@save/stale.png".into())
                .unwrap());
            assert_eq!(newer, serde_json::to_value(read().unwrap()).unwrap());
        }
    ...
    fn protected_slots_preserve_data_until_explicitly_unlocked() {
        ...
        let state = sample_state();
        mgr.save(&state, 1, "Original".into(), "test.rvn".into())
            .unwrap();
        mgr.set_protected(1, true).unwrap();
        let reopened = SaveManager::new(&dir, 10).unwrap();
        assert!(reopened.is_protected(1).unwrap());
        assert!(matches!(
            reopened.save(&state, 1, "Replacement".into(), "test.rvn".into()),
            Err(SaveError::SlotProtege(1))
        ));
        ...
        reopened
            .save(&state, 1, "Replacement".into(), "test.rvn".into())
            .unwrap();
    ...
    fn test_save_manager_write_read_delete() {
        ...
        mgr.set_thumbnail(1, "@save/thumbnail_1.png".into())
            .unwrap();
        let mut after = serde_json::to_value(mgr.load(1).unwrap()).unwrap();
        after["thumbnail"] = before["thumbnail"].clone();
        assert_eq!(
            before, after,
            "A thumbnail must not change narrative data or timestamp"
        );
