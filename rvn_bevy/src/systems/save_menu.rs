...
impl SaveConfirmation {
    pub fn active(&self) -> bool {
        self.pending.is_some() || self.action_pending.is_some()
    }
}
...
pub fn spawn_save_menu_overlay(
    ...
    let save_mgr = match SaveManager::new(
        &project_paths.saves,
        ...
        interaction_query.iter_mut()
    {
        ...
        match interaction {
            Interaction::Hovered => {
                *bg_color = colors
                    .map(|v| v.hover)
                    .unwrap_or(Color::srgba(0.20, 0.20, 0.40, 0.95))
                    .into();
            }
            Interaction::None => {
                *bg_color = colors
                    .map(|v| v.normal)
                    .unwrap_or(Color::srgba(0.10, 0.10, 0.25, 0.95))
                    .into();
            }
            Interaction::Pressed => {
                *bg_color = colors
                    .map(|v| v.pressed)
                    .unwrap_or(Color::srgba(0.30, 0.30, 0.50, 0.95))
                    .into();
                if cancel_comp.is_some() {
                    if confirmation.pending.take().is_some() {
                        return;
                    }
                ...
                if let Some(slot) = slot_comp {
                    ...
                    let occupied = SaveManager::new(&project_paths.saves, SUPPORTED_SLOTS)
                        .ok()
                        .is_some_and(|m| m.load(slot_index as u32).is_ok());
                    if !approved
                        && occupied
                        && mode != SaveMenuMode::ToggleProtection
                        && (matches!(mode, SaveMenuMode::Save | SaveMenuMode::Delete)
                            || save_state.origin == SaveMenuOrigin::InGame)
                    {
                        confirmation.pending = Some(operation);
                        return;
                    }
                    // Create SaveManager
                        ...
                        Ok(mgr) => {
                            match mode {
                                ...
                                SaveMenuMode::Save => {
                                    ...
                                    {
                                        Ok(_) => {
                                            if let Err(e) = thumbnails.persist(
                                                &engine,
                                                &mgr,
                                                &project_paths.saves,
                                                slot_index as u32,
                                            ) {
                                                warn!("[save_menu] miniature : {e}");
                                            }
...
mod tests {
    ...
    fn existing_save_requires_exact_one_shot_approval() {
        ...
        let engine = rvn_core::Engine::new(
        ...
        .unwrap();
        engine
            .save(&manager, 1, "Original protégé".into(), "test.rvn".into())
            .unwrap();
        let mut app = App::new();
        ...
        app.update();
        assert_eq!(
            app.world().resource::<SaveConfirmation>().pending,
            Some((1, SaveMenuMode::Save))
        );
        ...
        assert_eq!(manager.load(1).unwrap().label, "Original protégé");
        {
            let mut confirm = app.world_mut().resource_mut::<SaveConfirmation>();
            confirm.pending = None;
            confirm.approved = Some((1, SaveMenuMode::Save));
        }
        app.world_mut()
            .entity_mut(button)
            .insert(Interaction::Pressed);
        ...
        app.world_mut()
            .entity_mut(button)
            .insert((Interaction::Pressed, SaveSlotMode(SaveMenuMode::Load)));
        app.update();
        ...
        app.world_mut().entity_mut(button).insert((
            Interaction::Pressed,
            SaveSlotMode(SaveMenuMode::ToggleProtection),
        ));
        app.update();
        assert!(manager.is_protected(1).unwrap());
        assert!(app.world().resource::<SaveConfirmation>().pending.is_none());
        app.world_mut().resource_mut::<SaveConfirmation>().approved =
            Some((1, SaveMenuMode::Delete));
        app.world_mut()
            .entity_mut(button)
            .insert((Interaction::Pressed, SaveSlotMode(SaveMenuMode::Delete)));
        app.update();
        ...
        assert!(manager.slot_occupied(1));
        {
            let mut confirm = app.world_mut().resource_mut::<SaveConfirmation>();
            confirm.pending = None;
            confirm.approved = Some((1, SaveMenuMode::Delete));
        }
        ...
        assert!(app
            .world()
            .resource::<PersistentDataResource>()
            .data
            .last_resume_target
            .is_none());
        assert!(app.world().resource::<SaveMenuState>().active);
