...
pub(super) fn update(
    ...
    *hovering=next_hover;
    ...
    let title=if let Some((slot,mode))=operation{if slot==0{"Charger la sauvegarde rapide et remplacer la partie en cours ?".into()}else if mode==SaveMenuMode::Delete{format!("Supprimer définitivement la sauvegarde {slot} ?")}else if mode==SaveMenuMode::Save{format!("Remplacer la sauvegarde {slot} ?")}else{format!("Charger la sauvegarde {slot} et remplacer la partie en cours ?")}}else if matches!(action,Some((Action::Continue,_,_))){"Continuer la sauvegarde et remplacer la partie en cours ?".into()}else{"Démarrer une nouvelle partie et remplacer la partie en cours ?".into()};
