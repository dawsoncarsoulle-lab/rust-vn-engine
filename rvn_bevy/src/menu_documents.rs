...
pub(crate) fn web_menus(doc: Option<Document>) -> Result<Menus, String> {
    ...
    if let Some(doc) = doc {
        menus.session.initialize_controls(&doc)?;
        menus.doc = Some(doc);
    }
    Ok(menus)
}
#[path = "menu_animations.rs"]
mod animations;
#[path = "menu_confirmation.rs"]
mod confirmation;
#[path = "menu_dropdown.rs"]
mod dropdown;
#[path = "menu_dynamic_rows.rs"]
mod dynamic_rows;
#[path = "menu_image_states.rs"]
mod image_states;
...
mod narrative;
#[path = "menu_scrollbars.rs"]
mod scrollbars;
#[path = "menu_shadows.rs"]
mod shadows;
#[path = "menu_text_states.rs"]
mod text_states;
...
fn report_preview_error(session: &rvn_ui::Session) {
    ...
    let Some(path) = std::env::var_os("RVN_UI_DIAGNOSTICS") else {
        return;
    };
    if let Some(diagnostic) = &session.last_error {
        if let Ok(data) = serde_json::to_vec(diagnostic) {
            if let Err(error) = std::fs::write(path, data) {
                warn!("Diagnostic d’aperçu : {error}");
            }
        }
    }
}
impl Plugin for MenuDocumentsPlugin {
    fn build(&self, app: &mut App) {
        app.init_resource::<Menus>()
            ...
            .add_systems(
                ...
                narrative::render_narrative
                    ...
                    .after(crate::systems::apply_theme_system)
                    .after(render),
            )
            .add_systems(
                Update,
                narrative::render_quick_actions
                    .after(render)
                    .before(keyboard),
            )
            .add_systems(
            ...
            .add_systems(
                Update,
                (
                    narrative::render_choices,
                    narrative::choice_focus_visuals,
                    narrative::choice_scroll_bounds,
                )
                    .chain()
                    .after(crate::systems::update_choice_buttons)
                    .after(crate::systems::choice_interaction_system)
                    .before(scroll_lists),
            )
        ...
        app.add_systems(
            PostUpdate,
            (animations::transform, animations::fade)
                .chain()
                .after(bevy::ui::UiSystem::Layout)
        ...
        app.add_systems(
            PostUpdate,
            (
                dropdown::reveal_selection,
                dynamic_rows::grow,
                auto_list_bounds,
                narrative::follow_dialogue,
                scroll_to_focus,
                scrollbars::create,
                scrollbars::sync,
            )
...
impl Menus {
    ...
    pub(crate) fn choices_interactive(&self) -> bool {
        let Some(doc) = &self.doc else { return true };
        let Some(page) = doc
            .pages
            .iter()
        ...
        self.session
            .present(doc)
            .layout_page(page, doc.reference)
            .iter()
            .any(|e| e.kind == Kind::ChoiceList && e.enabled)
    }
}
...
fn gamepad_input(
    ...
    for button in pressed {
        let key = match button.button_type {
            ...
            GamepadButtonType::DPadLeft => KeyCode::ArrowLeft,
            GamepadButtonType::DPadRight => KeyCode::ArrowRight,
            GamepadButtonType::South => KeyCode::Enter,
            GamepadButtonType::East => KeyCode::Escape,
            _ => continue,
        };
        if !keys.pressed(key) {
            keys.press(key);
        ...
        pads.clear_just_pressed(button);
    }
}
impl Menus {
    pub(crate) fn custom_choices(&self) -> bool {
        self.doc.as_ref().is_some_and(|d| {
            d.pages
                .iter()
                .any(|p| p.role == Some(rvn_ui::PageRole::Choices))
        })
    }
}
#[derive(Component)]
pub(crate) struct MenuSlider(pub(crate) String);
fn slider_range(binding: &str) -> Option<(f32, f32)> {
    match binding {
        "music_volume" | "sfx_volume" => Some((0.0, 1.0)),
        "text_speed" | "auto_speed" => Some((0.1, 5.0)),
        _ => None,
    }
}
fn sliders(
    ...
    query: Query<(&Interaction, &MenuSlider, &Node, &GlobalTransform)>,
    focused: Query<&MenuSlider>,
    mut values: ResMut<crate::systems::settings_menu::Settings>,
    ...
    let direction = if keys.just_pressed(KeyCode::ArrowLeft)
        || pads
            .get_just_pressed()
            .any(|p| p.button_type == GamepadButtonType::DPadLeft)
    {
        -1.0
    } else if keys.just_pressed(KeyCode::ArrowRight)
        || pads
            .get_just_pressed()
            .any(|p| p.button_type == GamepadButtonType::DPadRight)
    {
        1.0
    } else {
        0.0
    ...
    if direction != 0.0 {
        if let Some(slider) = menus.focus.and_then(|e| focused.get(e).ok()) {
            if let Some((min, max)) = slider_range(&slider.0) {
                let slot = match slider.0.as_str() {
                    "music_volume" => &mut values.music_volume,
                    "sfx_volume" => &mut values.sfx_volume,
                    "text_speed" => &mut values.text_speed,
                    _ => &mut values.auto_speed,
                };
                *slot = (*slot + direction * (max - min) * 0.05).clamp(min, max);
                keyboard_changed = true;
            }
        }
    }
    if mouse.just_pressed(MouseButton::Left) {
        for (interaction, slider, node, transform) in &query {
            if *interaction == Interaction::Pressed {
                let left = transform.translation().x - node.size().x * 0.5 + 12.0;
                menus.slider_drag = Some((slider.0.clone(), left, (node.size().x - 24.0).max(1.0)));
                break;
            }
        }
    }
    if mouse.pressed(MouseButton::Left) {
        if let (Some((binding, left, width)), Some(cursor)) = (
            &menus.slider_drag,
            windows.get_single().ok().and_then(|w| w.cursor_position()),
        ) {
            if let Some((min, max)) = slider_range(binding) {
                let value = min + (max - min) * ((cursor.x - left) / width).clamp(0.0, 1.0);
                match binding.as_str() {
                    "music_volume" => values.music_volume = value,
                    "sfx_volume" => values.sfx_volume = value,
                    "text_speed" => values.text_speed = value,
                    "auto_speed" => values.auto_speed = value,
                    _ => {}
                }
            }
        }
    ...
    if (!mouse.pressed(MouseButton::Left) && menus.slider_drag.take().is_some()) || keyboard_changed
    {
        ...
        persistent.data.sfx_volume = Some(values.sfx_volume);
        persistent.data.text_speed = Some(values.text_speed);
        persistent.data.auto_speed = Some(values.auto_speed);
        if let Err(e) = persistent.manager.save(&persistent.data) {
            error!("[menus] Enregistrement du réglage impossible : {e}");
        }
    }
}
#[derive(Component)]
...
fn focus_visuals(
    ...
    menus: Res<Menus>,
    query: Query<(Entity, &FocusAppearance)>,
) {
...
fn paginate(
    ...
    for (interaction, button) in &buttons {
        if *interaction == Interaction::Pressed {
            menus.list_pages.insert(button.owner.clone(), button.page);
            menus.key.clear();
        }
    }
}
...
struct AutoList;
fn auto_list_bounds(
    mut lists: Query<(&Children, &Style, &mut MenuScroll), With<AutoList>>,
    nodes: Query<&Node, Without<scrollbars::Scrollbar>>,
) {
    for (children, style, mut scroll) in &mut lists {
        let gap = if let Val::Px(gap) = style.row_gap {
            gap
        } else {
            0.0
        };
        scroll.content_height = children
            .iter()
            .filter_map(|id| nodes.get(*id).ok())
            .map(|n| n.size().y)
            .sum::<f32>()
            + gap
                * children
                    .iter()
                    .filter(|id| nodes.get(**id).is_ok())
                    .count()
...
fn scroll_to_focus(
    ...
    mut lists: Query<(&Children, &mut MenuScroll)>,
    mut styles: Query<&mut Style, Without<scrollbars::Scrollbar>>,
) {
    if menus.focus == *previous {
        return;
    }
    *previous = menus.focus;
    let Some(target) = menus.focus else {
        return;
    };
    let Ok((target_node, target_transform)) = nodes.get(target) else {
        return;
    };
    let top = target_transform.translation().y - target_node.size().y * 0.5;
    let bottom = top + target_node.size().y;
    ...
    for _ in 0..64 {
        ...
        ancestor = parent.get();
        let Ok((children, mut scroll)) = lists.get_mut(ancestor) else {
            continue;
        };
        let Ok((node, transform)) = nodes.get(ancestor) else {
            continue;
        };
        let origin = transform.translation().y - node.size().y * 0.5;
        let delta = if top < origin {
            top - origin
        } else {
            (bottom - origin - node.size().y).max(0.0)
        };
        ...
        for child in children {
            if let Ok(mut style) = styles.get_mut(*child) {
                if let Val::Px(y) = style.top {
                    style.top = Val::Px(y + old - scroll.offset);
                }
            }
        }
    }
...
fn keyboard(
    ...
    if menus.focus.is_some_and(|entity| query.get(entity).is_err()) {
        menus.focus = items
            .iter()
            .find(|(_, _, key)| Some(key) == menus.focus_key.as_ref())
    ...
    if keys.just_pressed(KeyCode::Escape) || pad(GamepadButtonType::East) {
        if let Some(page) = menus.active_page.clone() {
            keys.clear_just_pressed(KeyCode::Escape);
            menus
                .activations
                .push((page, None, rvn_ui::Event::Click, Action::Back));
            ...
            && (backward || keys.just_pressed(KeyCode::ArrowDown)))
    {
        ...
        if let Some((page, id)) = items[index].2.split_once('/') {
            ...
            if menus
                .doc
                .as_ref()
                .is_some_and(|doc| doc.pages.iter().any(|p| p.id == page))
            {
                menus.activations.push((
                    page.into(),
                    Some(id.into()),
                    rvn_ui::Event::Focus,
                    Action::None,
...
fn scroll_lists(
    ...
    let contains = |node: &Node, transform: &GlobalTransform| {
        let origin = transform.translation().truncate() - node.size() * 0.5;
        cursor.x >= origin.x
            && cursor.y >= origin.y
            && cursor.x < origin.x + node.size().x
            && cursor.y < origin.y + node.size().y
    };
    let target = lists
        .iter()
        .filter(|(_, n, t, _, s)| {
            contains(n, t)
                && (s.offset - delta).clamp(0.0, (s.content_height - n.size().y).max(0.0))
                    != s.offset
        })
        .filter_map(|(id, _, _, _, _)| {
            ...
            while let Ok(parent) = parents.get(at) {
                at = parent.get();
                depth += 1;
                in_popup |= popups.contains(at);
                if depth > 64 {
                    return None;
                }
                if let Ok((_, n, t, _, _)) = lists.get(at) {
                    if !contains(n, t) {
                        return None;
                    }
                }
            }
            if dropdown.open && !in_popup {
                return None;
            }
            Some((depth, id))
        })
    ...
    if let Some(target) = target {
        if let Ok((_, node, _, children, mut scroll)) = lists.get_mut(target) {
            let old = scroll.offset;
            scroll.offset =
                (old - delta).clamp(0.0, (scroll.content_height - node.size().y).max(0.0));
...
fn load(paths: Res<ProjectPaths>, mut menus: ResMut<Menus>, engine: Res<VnEngine>) {
    ...
    match result {
        Ok(doc) => {
            ...
            if let Err(error) = menus.session.initialize_controls(&doc) {
                error!("Contrôles d’interface : {error}");
                return;
            }
            menus.doc = Some(doc);
            ...
            menus.focus_key = None;
            menus.pressed = None;
            menus.list_pages.clear();
            menus.page = None;
            menus.history.clear();
            menus.pending = None;
            menus.overlay_return = None;
            menus.key.clear();
            menus.error.clear();
        }
        Err(e) => {
...
fn translated(ctx: &Context, text: &str) -> String {
    ctx.engine
        ...
        .locale
        .as_ref()
        .map(|locale| locale.translate(text))
        .unwrap_or(text)
        .to_owned()
}
fn render(
    ...
    let page_id = menus.page.clone().unwrap_or_else(|| {
        menus
            ...
            .and_then(|d| {
                d.pages
                    .iter()
                    .find(|p| p.role.is_some() && p.role == rvn_ui::PageRole::from_id(base))
            })
            .map(|p| p.id.clone())
            .unwrap_or_default()
    });
    ...
    let settings_values: std::collections::BTreeMap<String, serde_json::Value> = [
        ...
        ("sfx_volume", serde_json::json!(ctx.values.sfx_volume)),
        ("text_speed", serde_json::json!(ctx.values.text_speed)),
        ("auto_speed", serde_json::json!(ctx.values.auto_speed)),
        ("typewriter", serde_json::json!(ctx.values.typewriter)),
        ("fullscreen", serde_json::json!(ctx.values.fullscreen)),
        ("language", serde_json::json!(ctx.values.language)),
    ]
    .into_iter()
    .map(|(k, v)| (k.into(), v))
    .collect();
    if active {
        if let Some(page) = &page {
            let changed: std::collections::BTreeSet<_> = page
                .graphs
                .iter()
                .filter(|g| g.event == rvn_ui::Event::ValueChanged)
                .filter_map(|g| {
                    let id = g.target.as_ref()?;
                    let doc = menus.doc.as_ref()?;
                    let index = doc.pages.iter().position(|p| p.id == page.id)?;
                    let element = doc.resolved_element(doc.find_element(index, id)?);
                    let binding = element.binding.as_ref()?;
                    let old = menus.settings_values.get(binding)?;
                    let value = settings_values.get(binding)?;
                    if old != value {
                        Some(id.clone())
                    } else {
                        None
            ...
            for id in changed {
                menus.activations.push((
                    page.id.clone(),
                    Some(id),
                    rvn_ui::Event::ValueChanged,
                    Action::None,
                ));
            }
        }
    }
    for (binding, value) in &settings_values {
        menus
            .session
            .variables
            .insert(format!("state.settings.{binding}"), value.clone());
    }
    menus.settings_values = settings_values;
    let active_page = if active { Some(page_id.clone()) } else { None };
    if menus.active_page != active_page {
        if let Some(old) = menus.active_page.take() {
            menus.page_events.push((old, rvn_ui::Event::Close));
        }
        if let Some(new) = &active_page {
            menus.page_events.push((new.clone(), rvn_ui::Event::Open));
        }
        menus.active_page = active_page;
        menus.hovering.clear();
    }
    ...
    let Ok(window) = ctx.windows.get_single() else {
        return;
    };
    let size = [window.width(), window.height()];
    let doc = menus.session.present(menus.doc.as_ref().unwrap());
    let image_state: Vec<_> = doc
        .resource_paths()
        .into_iter()
        .filter_map(|path| {
            assets.get_handle::<Image>(path).and_then(|handle| {
                ctx.images
                    .get(&handle)
                    .map(|image| (handle.id(), image.size()))
            })
        })
        .collect();
    let key = format!(
        ...
        ctx.values.language,
        format!("{:?}", menus.list_pages)
    );
    let presentation: Vec<_> = menus
        .session
        .presentation
        .iter()
        .map(|(id, s)| (id, s.visible, s.enabled, &s.text, &s.image))
        .collect();
    let locals: Vec<_> = menus
        .session
        .variables
        .iter()
        ...
        .collect();
    let key = format!(
        "{key}:{image_state:?}:{}:{:?}:{presentation:?}:{locals:?}:{}",
        ctx.values.auto_speed,
        (ctx.images.len(), ctx.fonts.len(), ctx.save.revision),
        ctx.engine
            .0
            .locale
            .as_ref()
    ...
    for element in &menu_layout::tree(
        &doc,
        page_index,
        size,
        &assets,
        &ctx,
        menus.list_pages.get(&page.id).copied().unwrap_or(0),
        &[],
    ) {
        spawn_element(
            &mut commands,
...
fn spawn_element(
    ...
    let selected = match &e.action {
        ...
        Action::OpenPage(id) => id == page,
        Action::Save => role == Some(rvn_ui::PageRole::Save),
        Action::Load => role == Some(rvn_ui::PageRole::Load),
        Action::Settings => role == Some(rvn_ui::PageRole::Settings),
        Action::History => role == Some(rvn_ui::PageRole::History),
        Action::Gallery => role == Some(rvn_ui::PageRole::Gallery),
        _ => {
            e.kind == Kind::CheckBox
                && match e.binding.as_deref() {
                    Some("typewriter") => ctx.values.typewriter,
                    Some("fullscreen") => ctx.values.fullscreen,
                    _ => e
                        .local_control
                        .as_ref()
                        .and_then(|c| c.initial.as_bool())
                        .unwrap_or(false),
                }
        }
    };
    let mut selected_visual;
    let e = if selected && e.appearance.selected.is_some() {
        selected_visual = e.clone();
        selected_visual.normal = e.appearance.selected.unwrap();
        &selected_visual
    } else {
        e
    };
    ...
    let style = Style {
        ...
        width: Val::Px(r[2]),
        height: Val::Px(r[3]),
        align_items: if e.kind == Kind::Button {
            AlignItems::Center
        } else {
            AlignItems::FlexStart
        },
        justify_content: JustifyContent::FlexStart,
        padding: UiRect::all(Val::Px(if e.kind == Kind::Text {
            0.0
        } else {
            (e.font_size * 0.15).min(6.0)
        })),
        overflow: if matches!(
            e.kind,
            Kind::Panel | Kind::Horizontal | Kind::Vertical | Kind::Grid | Kind::Overlay
        ) {
            Overflow::visible()
        } else {
            Overflow::clip()
    ...
    let page_action_enabled = if doc
        .pages
        .iter()
        .filter(|p| p.id == page)
        .flat_map(|p| &p.graphs)
        .any(|g| g.target.as_deref() == Some(e.id.as_str()) && g.event == rvn_ui::Event::Click)
    {
        true
    } else if let Action::SavePage(target) = &e.action {
        matches!(target, rvn_ui::SavePage::Number(_))
            || doc
                .pages
                .iter()
                .position(|p| p.id == page)
                .and_then(|i| doc.save_page_count(i))
                .is_some_and(|count| target.resolve(list_page, count) != list_page)
    } else {
        true
    };
    ...
    commands
        .entity(entity)
        .insert(scrollbars::ScrollAppearance::from_element(e));
    commands.entity(parent).add_child(entity);
    commands.entity(entity).insert(animations::VisualNode {
        page: page.into(),
        id: e.id.clone(),
        scale: (ctx.windows.single().width() / doc.reference[0])
            .min(ctx.windows.single().height() / doc.reference[1]),
    });
    attach_interactions(commands, entity, e, page, doc, enabled);
    if enabled {
        if let Some((kind, slot)) = e.binding.as_deref().and_then(|b| b.split_once(':')) {
            if let Ok(slot) = slot.parse::<usize>() {
                let mode = match kind {
                    "save.save" => Some(SaveMenuMode::Save),
                    "save.load" => Some(SaveMenuMode::Load),
                    "save.delete" => Some(SaveMenuMode::Delete),
                    "save.protect" => Some(SaveMenuMode::ToggleProtection),
                    _ => None,
                };
    ...
    if !matches!(
        ...
        let value = value.or_else(|| {
            e.local_control.as_ref().map(|c| {
                c.initial
                    .as_str()
                    .map(str::to_owned)
                    .unwrap_or_else(|| c.initial.to_string())
            })
        });
        let label = value
        ...
        commands.entity(entity).with_children(|p| {
            ...
            .with_text_justify(match e.layout_options.text_align {
                rvn_ui::TextAlign::Left => JustifyText::Left,
                rvn_ui::TextAlign::Center => JustifyText::Center,
                rvn_ui::TextAlign::Right => JustifyText::Right,
            });
            text.style.width = Val::Percent(100.0);
            if matches!(e.kind, Kind::CheckBox | Kind::Select) {
                text.style.padding.right = Val::Px(34.0);
            }
            if !e.layout_options.text_wrap {
                text = text.with_no_wrap();
            }
            p.spawn((text, text_states::TextAppearance::new(e, selected)));
        });
    }
    if e.kind == Kind::CheckBox {
        let checked = match e.binding.as_deref() {
            Some("typewriter") => ctx.values.typewriter,
            Some("fullscreen") => ctx.values.fullscreen,
            _ => e
                .local_control
        ...
        let side = e.font_size.clamp(16.0, 28.0);
        commands.entity(entity).with_children(|p| {
            p.spawn(NodeBundle {
                style: Style {
                    position_type: PositionType::Absolute,
                    right: Val::Px(8.0),
                    top: Val::Px((r[3] - side) * 0.5),
                    width: Val::Px(side),
                    height: Val::Px(side),
                ...
                border_color: color(e.foreground).into(),
                background_color: if checked { color(e.hover) } else { Color::NONE }.into(),
                ..default()
            })
            .with_children(|p| {
                if checked {
        ...
        commands.entity(entity).with_children(|p| {
            p.spawn(TextBundle {
                style: Style {
                    position_type: PositionType::Absolute,
                    right: Val::Px(10.0),
                    top: Val::Px((r[3] - e.font_size) * 0.5),
                    ..default()
                },
    ...
    if e.kind == Kind::SaveList {
        let automatic = !doc
            ...
            .iter()
            .position(|p| p.id == page)
            .is_some_and(|i| doc.custom_save_pagination(i));
        let mgr = rvn_core::save::SaveManager::new(
            &ctx.paths.saves,
            crate::systems::save_menu::SUPPORTED_SLOTS,
        )
        .ok();
        let capacity = e.list.columns * e.list.rows;
        let pages = e.list.slots.div_ceil(capacity);
        let current = list_page.min(pages.saturating_sub(1));
        let mut card_layouts = Vec::new();
        for (index, slot) in ((current * capacity + 1)
            ..=(current * capacity + capacity).min(e.list.slots))
            .enumerate()
        {
            let saved = mgr.as_ref().and_then(|m| m.load(slot as u32).ok());
            let present = saved.is_some();
            let protected = mgr
                .as_ref()
                .is_none_or(|m| m.is_protected(slot as u32).unwrap_or(true));
            let label = format!(
                "{} — {}",
                slot,
                translated(ctx, if present { "Sauvegarde" } else { "Vide" })
            );
            let mut explicit_controls = false;
            let b = if let Some(template) = &e.list.template {
                let bounds = e.list.card_rect_with_navigation(
                    index,
                    [r[2], r[3]],
                    e.layout_options.gap,
                    automatic,
                );
                let card = [bounds[2], bounds[3]];
                let root = commands
                    .spawn(ButtonBundle {
                        style: Style {
                            position_type: PositionType::Absolute,
                            left: Val::Px(bounds[0]),
                            top: Val::Px(bounds[1]),
                            width: Val::Px(card[0]),
                            height: Val::Px(card[1]),
                ...
                let date = saved
                    .as_ref()
                    .and_then(|s| chrono::DateTime::from_timestamp(s.timestamp as i64, 0))
                    .map(|d| {
                        d.with_timezone(&chrono::Local)
                            .format("%d/%m/%Y %H:%M")
                            .to_string()
                    })
                    .unwrap_or_else(|| "Emplacement vide".into());
                let data = [
                    ("save.slot".into(), slot.to_string()),
                    (
                        "save.thumbnail".into(),
                        saved
                            .as_ref()
                    ...
                    (
                        "save.summary".into(),
                        saved.as_ref().map(|s| s.label.clone()).unwrap_or_else(|| {
                            format!("{} {slot}", translated(ctx, "Emplacement"))
                        }),
                    ),
                ]
                .into_iter()
                .collect();
                let mut data: std::collections::BTreeMap<String, String> = data;
                data.extend(rvn_ui::card_state_bindings(false, present, protected));
                let children = menu_layout::item(doc, template, card, &data, assets, ctx);
                ...
                for mut child in children {
                    ...
                    if child.kind == Kind::Button {
                        if let Some(binding) = child.binding.clone().filter(|b| {
                            matches!(
                                b.as_str(),
                                "save.save" | "save.load" | "save.delete" | "save.protect"
                            )
                        }) {
                            ...
                            child.enabled &= enabled
                                && match binding.as_str() {
                                    ...
                                    "save.delete" => present && !protected,
                                    _ => present,
                                };
                            if binding == "save.protect" && protected {
                                child.text = "Déprotéger".into();
                            }
                            child.binding = Some(format!("{binding}:{slot}"));
                            child.id = format!("{}/{slot}/{}", e.id, child.id);
                        }
                    }
                    spawn_element(
                        commands, root, &child, page, card, card, assets, ctx, doc, current,
                    );
                }
                root
            } else {
            ...
            if enabled
                && !explicit_controls
                && (present || ctx.save.mode == SaveMenuMode::Save)
                && !(protected && ctx.save.mode == SaveMenuMode::Save)
            {
                commands.entity(b).insert((
                    SaveSlotButton(slot),
                    MenuFocus(
                        e.focus_order + slot as i32,
                        format!("{page}/{}/slot/{slot}", e.id),
                    ),
                    FocusAppearance(color(e.appearance.focus.unwrap_or(e.hover))),
                    crate::systems::save_menu::SaveSlotColors {
                        normal: color(e.normal),
                        hover: color(e.hover),
                        pressed: color(e.pressed),
                    },
                ));
            }
        }
        let mut fitted: Vec<_> = card_layouts.iter().map(|(_, b, _)| *b).collect();
        rvn_ui::fit_card_rows(
            &mut fitted,
            &card_layouts.iter().map(|(_, _, h)| *h).collect::<Vec<_>>(),
            e.layout_options.gap,
        );
        let bottom = fitted.iter().map(|b| b[1] + b[3]).fold(0.0, f32::max);
        for ((card, _, _), bounds) in card_layouts.iter().zip(&fitted) {
            commands.entity(*card).insert(Style {
                position_type: PositionType::Absolute,
                left: Val::Px(bounds[0]),
                top: Val::Px(bounds[1]),
                width: Val::Px(bounds[2]),
                height: Val::Px(bounds[3]),
                overflow: Overflow::clip(),
                ..default()
            });
        }
        ...
        if !fitted.is_empty() {
            commands.entity(entity).insert(MenuScroll {
                offset: 0.0,
                content_height: if automatic && pages > 1 {
                    footer + 36.0
                } else {
                    bottom
                },
        ...
        if automatic && pages > 1 {
            for (x, label, target) in [
                (0.0, "Précédent", current.saturating_sub(1)),
                (
                    (r[2] - 120.0).max(0.0),
                    "Suivant",
                ...
                let b = list_button(commands, entity, label, 0, 120.0, e, assets, &ctx.font.0);
                commands.entity(b).insert(Style {
                    position_type: PositionType::Absolute,
                    left: Val::Px(x),
                    top: Val::Px(footer),
                    width: Val::Px(120.0),
                ...
                if enabled {
                    commands.entity(b).insert((
                        ...
                        MenuFocus(10000 + x as i32, format!("{page}/{}/page/{x}", e.id)),
                        FocusAppearance(color(e.appearance.focus.unwrap_or(e.hover))),
                    ));
                }
            }
    ...
    if e.kind == Kind::History {
        commands.entity(entity).insert((
            ...
            Style {
                ...
                row_gap: Val::Px(e.layout_options.gap),
                overflow: Overflow::clip_y(),
                ..default()
            },
        ));
        for (i, line) in ctx.history.lines.iter().rev().enumerate() {
            let label = if line.character.is_empty() {
                line.text.clone()
            } else {
                format!("{} : {}", line.character, line.text)
            };
            if let Some(template) = &e.list.template {
                let data = doc.history_data(template, &line.character, &line.text);
                dynamic_rows::spawn(
                    commands, entity, e, page, template, &data, assets, ctx, doc, None,
                );
                continue;
            }
            let row = commands
    ...
    if e.kind == Kind::Gallery {
        let mut entries: Vec<_> = ctx.cgs.0.iter().collect();
        ...
        for (i, (id, path)) in entries.iter().enumerate() {
            ...
            let b = if let Some(template) = &e.list.template {
                let bounds = e.list.card_rect(i, [r[2], r[3]], e.layout_options.gap);
                ...
                let b = commands
                    ...
                    .id();
                commands.entity(entity).add_child(b);
                let data = [
                    (
                        "gallery.thumbnail".into(),
                        if unlocked {
                            (*path).clone()
                        } else {
                            String::new()
                ...
                .collect();
                let mut data: std::collections::BTreeMap<String, String> = data;
                data.extend(rvn_ui::card_state_bindings(true, true, !unlocked));
                let children = menu_layout::item(doc, template, size, &data, assets, ctx);
                if children.is_empty() {
                ...
                for mut child in children {
                    ...
                    spawn_element(commands, b, &child, page, size, size, assets, ctx, doc, 0);
                }
                b
            } else {
                list_button(
            ...
            if enabled && unlocked {
                commands.entity(b).insert((
                    ...
                    crate::systems::gallery::GalleryColors {
                        normal: color(e.normal),
                        hover: color(e.hover),
                        pressed: color(e.pressed),
                    },
        ...
        if e.list.template.is_some() {
            let mut bounds: Vec<_> = gallery_layouts.iter().map(|(_, r, _)| *r).collect();
            rvn_ui::fit_card_rows(
                &mut bounds,
                &gallery_layouts
                    .iter()
            ...
            for ((card, _, _), r) in gallery_layouts.iter().zip(&bounds) {
                commands.entity(*card).insert(Style {
                    position_type: PositionType::Absolute,
                    left: Val::Px(r[0]),
                    top: Val::Px(r[1]),
            ...
            commands.entity(entity).insert(MenuScroll {
                offset: 0.0,
                content_height: bounds.iter().map(|r| r[1] + r[3]).fold(0.0, f32::max),
            });
        }
    }
    ...
    if enabled && matches!(e.kind, Kind::CheckBox | Kind::Select) {
        ...
        if e.kind == Kind::Select
            && (e.binding.as_deref() == Some("language") || e.local_control.is_some())
        {
            commands.entity(entity).insert(dropdown::LanguageSelect {
                normal: color(e.normal),
                hover: color(e.hover),
                selected: color(e.appearance.selected.unwrap_or(e.hover)),
                foreground: color(e.foreground),
    ...
    if e.kind == Kind::Slider {
        let slider = e
            ...
            .map(|c| {
                (
                    c.initial.as_f64().unwrap_or(c.minimum) as f32,
                    c.minimum as f32,
                    c.maximum as f32,
                )
            })
            .or_else(|| {
                e.binding.as_deref().and_then(|binding| {
                    slider_range(binding).map(|(min, max)| {
                        let value = match binding {
                            "music_volume" => ctx.values.music_volume,
                            "sfx_volume" => ctx.values.sfx_volume,
                            "text_speed" => ctx.values.text_speed,
                            _ => ctx.values.auto_speed,
                        };
                        (value, min, max)
                    })
                })
        ...
        if let Some((value, min, max)) = slider {
            ...
            if enabled && e.local_control.is_none() {
                commands
                    .entity(entity)
                    .insert(MenuSlider(e.binding.clone().unwrap()));
            }
            commands.entity(entity).with_children(|p| {
                p.spawn(NodeBundle {
                    style: Style {
                        ...
                        left: Val::Px(12.0),
                        right: Val::Px(12.0),
                        bottom: Val::Px(8.0 * ui_scale),
                        height: Val::Px(4.0 * ui_scale),
                        ..default()
                    },
                    background_color: color(e.disabled).into(),
                    ..default()
                });
                p.spawn(NodeBundle {
                    style: Style {
                        ...
                        left: Val::Px(12.0),
                        bottom: Val::Px(8.0 * ui_scale),
                        width: Val::Px((r[2] - 24.0).max(1.0) * fraction),
                        height: Val::Px(4.0 * ui_scale),
                        ..default()
                    },
                    background_color: color(e.hover).into(),
                    ..default()
                });
                p.spawn(NodeBundle {
                    style: Style {
                        position_type: PositionType::Absolute,
                        left: Val::Px(7.0 + (r[2] - 24.0).max(1.0) * fraction),
                        bottom: Val::Px(3.0 * ui_scale),
                        width: Val::Px(10.0),
                        height: Val::Px(14.0 * ui_scale),
                        ..default()
                    },
                    background_color: color(e.foreground).into(),
                    ..default()
                });
            });
        }
    ...
    if e.kind == Kind::Scroll {
        fn height(es: &[Element], y: f32) -> f32 {
            es.iter()
                .map(|e| (y + e.rect[1] + e.rect[3]).max(height(&e.children, y + e.rect[1])))
                .fold(0.0, f32::max)
        }
        commands.entity(entity).insert(MenuScroll {
            offset: 0.0,
            content_height: height(&e.children, 0.0),
        });
    }
...
fn interaction_policy(kind: Kind, events: impl Iterator<Item = rvn_ui::Event>) -> (bool, bool) {
    let control = matches!(
        kind,
        Kind::Button | Kind::CheckBox | Kind::Slider | Kind::Select
    );
    let (mut pointer, mut focus) = (control, control);
    for event in events {
        match event {
            rvn_ui::Event::Click | rvn_ui::Event::Focus => {
                pointer = true;
                focus = true;
            }
...
#[derive(Component)]
pub(crate) struct UiInputBlocker;
fn attach_interactions(
    ...
    let events = doc
        ...
        .filter(|p| p.id == page)
        .flat_map(|p| &p.graphs)
        .filter(|g| g.target.as_deref() == Some(e.id.as_str()))
        .map(|g| g.event.clone())
        .chain((e.action != Action::None).then_some(rvn_ui::Event::Click));
    let (pointer, focus) = interaction_policy(e.kind.clone(), events);
    if pointer {
        commands.entity(entity).insert(bevy::ui::FocusPolicy::Block);
    }
    if !enabled {
        if pointer {
            commands
                .entity(entity)
                .insert((Interaction::None, UiInputBlocker));
        }
        return;
    }
    if let Some(control) = &e.local_control {
        commands.entity(entity).insert(local_controls::LocalInput {
            page: page.into(),
            id: e.id.clone(),
            kind: e.kind,
    ...
    if focus {
        commands.entity(entity).insert((
            Button,
            MenuFocus(e.focus_order, format!("{page}/{}", e.id)),
            FocusAppearance(color(e.appearance.focus.unwrap_or(e.hover))),
        ));
    }
}
...
#[cfg(test)]
mod interaction_policy_tests {
    #[test]
    fn disabled_controls_block_pointer_without_receiving_an_action_or_focus() {
        let mut world = World::new();
        let entity = world.spawn_empty().id();
        let mut queue = bevy::ecs::world::CommandQueue::default();
        let doc = Document::defaults();
        let mut e = Element::new("disabled".into(), Kind::Button);
        e.enabled = false;
        e.action = Action::NewGame;
        {
            let mut commands = Commands::new(&mut queue, &world);
            attach_interactions(&mut commands, entity, &e, "title", &doc, false);
        }
        queue.apply(&mut world);
        assert!(world.get::<UiInputBlocker>(entity).is_some());
        assert!(world.get::<Interaction>(entity).is_some());
        assert!(matches!(
            world.get::<bevy::ui::FocusPolicy>(entity),
            Some(bevy::ui::FocusPolicy::Block)
        ));
        assert!(world.get::<MenuElement>(entity).is_none());
        assert!(world.get::<MenuFocus>(entity).is_none());
    }
    #[test]
    fn hidden_or_disabled_choices_block_every_activation_path() {
        let mut menus = Menus::default();
        assert!(menus.choices_interactive());
        let mut doc = Document::defaults();
        let page = doc.add_choices_page();
        let page_id = doc.pages[page].id.clone();
        let list_id = doc.pages[page].elements[0].id.clone();
        let before = doc.clone();
        menus.doc = Some(doc);
        assert!(menus.choices_interactive());
        let key = (page_id, list_id);
        ...
        assert!(menus.choices_interactive());
        assert_eq!(menus.doc.as_ref().unwrap(), &before);
    }
    use super::*;
    #[test]
    fn decorative_regions_do_not_steal_focus() {
        assert_eq!(
            interaction_policy(Kind::Image, std::iter::empty()),
            (false, false)
        );
        assert_eq!(
            interaction_policy(
                Kind::Panel,
                [rvn_ui::Event::Hover, rvn_ui::Event::HoverLeave].into_iter()
            ),
            (true, false)
        );
        assert_eq!(
            interaction_policy(Kind::Image, [rvn_ui::Event::Click].into_iter()),
            (true, true)
        );
        assert_eq!(
            interaction_policy(Kind::Text, [rvn_ui::Event::Focus].into_iter()),
            (true, true)
        );
        assert_eq!(
            interaction_policy(Kind::Button, std::iter::empty()),
            (true, true)
        );
    }
    #[test]
    fn authored_nine_slice_matches_native_texture_slicer() {
        let image = Vec2::new(1920.0, 505.0);
        let border = [140.0, 90.0, 140.0, 90.0];
        let slicer = bevy::sprite::TextureSlicer {
        ...
        for size in [
            ...
            Vec2::new(2400.0, 800.0),
        ] {
            let native = slicer.compute_slices(Rect::from_corners(Vec2::ZERO, image), Some(size));
            let preview = rvn_ui::image_quads(
                image.to_array(),
                size.to_array(),
                rvn_ui::ImageFit::NineSlice,
                border,
            );
            assert_eq!(native.len(), preview.len());
            for (r, uv) in preview {
                let s = native
                    .iter()
                    .find(|s| {
                        (s.texture_rect.min.x / image.x - uv[0]).abs() < 0.00001
                            && (s.texture_rect.min.y / image.y - uv[1]).abs() < 0.00001
                    })
                    .unwrap();
                let actual = [
                    size.x * 0.5 + s.offset.x - s.draw_size.x * 0.5,
                    size.y * 0.5 - s.offset.y - s.draw_size.y * 0.5,
                    s.draw_size.x,
                    s.draw_size.y,
                ];
                for (a, b) in actual.into_iter().zip(r) {
                    assert!((a - b).abs() < 0.001, "{actual:?} != {r:?}");
                }
            }
        }
    }
...
fn interact(
    ...
    assets: Res<AssetServer>,
    values: Res<crate::systems::settings_menu::Settings>,
) {
    let mut triggers = Vec::new();
    if ctx.confirmation.active() {
        return;
    }
    for (interaction, e, mut bg) in &mut query {
        *bg = match interaction {
        ...
        if *interaction == Interaction::Pressed
            && menus.doc.as_ref().is_some_and(|d| {
                d.pages
                    .iter()
                    .position(|p| p.id == e.page)
                    .and_then(|p| d.find_element(p, &e.id))
                    .is_some_and(|e| d.resolved_element(e).local_control.is_some())
            })
        {
            continue;
        }
        let event = if *interaction == Interaction::None {
            if !menus.hovering.remove(&key) {
                continue;
            }
            rvn_ui::Event::HoverLeave
        } else if *interaction == Interaction::Pressed {
            menus.automatic_navigations = 0;
            rvn_ui::Event::Click
        } else {
            if !menus.hovering.insert(key) {
                continue;
            }
            rvn_ui::Event::Hover
        };
    ...
    for (page, event) in std::mem::take(&mut menus.page_events) {
        triggers.push((
            MenuElement {
                page,
                id: String::new(),
    ...
    for (page, target, event, action) in std::mem::take(&mut menus.activations) {
        triggers.push((
            MenuElement {
                page,
    ...
    for (e, target, event) in triggers {
        ...
        let Some(page) = menus
            .doc
            .as_ref()
            .and_then(|d| d.pages.iter().find(|p| p.id == e.page))
            .cloned()
        else {
            continue;
        };
        let effects =
            match menus
                .session
                .dispatch(&page, target.as_deref(), event.clone(), e.action.clone())
            {
                Ok(es) => es,
                Err(err) => {
                    error!("Menus : {err}");
                    report_preview_error(&menus.session);
                    continue;
                }
            };
        for effect in effects {
            if target.is_none() && matches!(&effect,Effect::Action(a) if *a!=Action::None) {
                menus.automatic_navigations += 1;
                if menus.automatic_navigations > 16 {
                    let message = "Boucle de navigation automatique interrompue (16 transitions)";
                    error!("{message}");
                    menus
                        .session
                        .action_diagnostic(&page, target.as_deref(), &event, message);
                    report_preview_error(&menus.session);
                    continue;
                }
            }
            match effect {
                Effect::Animate(id, clip) => {
                    menus.animation_requests.push((e.page.clone(), id, clip))
                }
                Effect::Sound(path) => {
                    commands.spawn(AudioBundle {
                        source: assets.load(path),
                        settings: bevy::audio::PlaybackSettings::DESPAWN
                            .with_volume(bevy::audio::Volume::new(values.sfx_volume)),
                        ..default()
                    });
                }
                ...
                Effect::Action(Action::SavePage(target)) => {
                    if let Some(index) = menus
                        .doc
                        .as_ref()
                        .and_then(|d| d.pages.iter().position(|p| p.id == e.page))
                    {
                        if let Some(count) =
                            menus.doc.as_ref().and_then(|d| d.save_page_count(index))
                        {
                            let current = menus.list_pages.get(&e.page).copied().unwrap_or(0);
                            menus
                                .list_pages
                                .insert(e.page.clone(), target.resolve(current, count));
                            menus.key.clear();
                        }
                    }
                }
                Effect::Action(Action::OpenPage(id)) => {
                    ...
                    if *ctx.state.get() == VnState::Waiting {
                        menus.overlay_return = Some(VnState::Waiting);
                ...
                Effect::Action(Action::Back) if menus.page.is_some() => {
                    ...
                    if menus.page.is_none() {
                        if let Some(ret) = menus.overlay_return.take() {
                            ctx.next.set(ret);
                        }
                    }
                    menus.key.clear();
                ...
                Effect::Action(mut a) => {
                    ...
                    let quick = match a {
                        ...
                        Action::QuickLoad => Some(PlayerInput::QuickLoad),
                        Action::Rollback => Some(PlayerInput::Rollback),
                        Action::ToggleMenu => Some(PlayerInput::ToggleMenu),
                        _ => None,
                    };
                    if let Some(input) = quick {
                        if *ctx.state.get() == VnState::Waiting {
                    ...
                    if matches!(
                        a,
                        Action::NewGame | Action::StartScene(_) | Action::Continue
                    ) && *ctx.state.get() != VnState::TitleScreen
                        && ctx.menu.return_to != Some(VnState::TitleScreen)
                        && !approved
                    {
                        menus.pending = Some(a.clone());
                        ctx.confirmation.action_pending =
                            Some((a, e.page.clone(), ctx.state.get().clone()));
                        if *ctx.state.get() == VnState::Waiting {
                            ctx.next.set(VnState::Menu);
                        }
                        continue;
                    ...
                    if a == Action::Continue {
                        ...
                        .and_then(|manager| {
                            crate::systems::title::resolve_continue_data(&manager, &ctx.persistent)
                        });
                        match result {
                            Ok((data, target)) => {
                                ...
                                if let Some(target) = target {
                                    ctx.persistent.data.last_resume_target = Some(target);
                                    if let Err(e) =
                                        ctx.persistent.manager.save(&ctx.persistent.data)
                                    {
                                        error!("Reprise : {e}");
                                    }
                                }
                    ...
                    if matches!(
                        ctx.state.get(),
                        VnState::Menu | VnState::Gallery | VnState::History
                    ) && matches!(a, Action::Save | Action::Load | Action::Settings)
                    {
                        ...
                        match a {
                            ...
                            _ => {
                                ...
                                ctx.save.open(
                                    ...
                                    if from_title {
                                        crate::systems::save_menu::SaveMenuOrigin::TitleScreen
                                    } else {
                                        crate::systems::save_menu::SaveMenuOrigin::InGame
                                    },
                                );
                    ...
                    if ctx.save.active && a == Action::Back {
                        ...
                        let return_to = if ctx.menu.return_to == Some(VnState::TitleScreen) {
                            VnState::TitleScreen
                        } else {
                            VnState::Menu
                        };
                        ...
                        ctx.next
                            .set(ctx.menu.return_to.take().unwrap_or(VnState::Waiting));
