...
use crate::systems::{
    save_menu::{SaveMenuMode, SaveMenuOrigin, SaveMenuState},
...
pub(super) fn drive(
    ...
    if initial_window.is_empty() {
        ...
        let config: toml::Value =
            toml::from_str(&std::fs::read_to_string(paths.root.join("rvn.toml")).unwrap()).unwrap();
        ...
        *initial_window = "requested".into();
        phase.1 = now;
        return;
    }
    let mut capture = |name: &str| {
        shots
            .save_screenshot_to_disk(window, qa.dir.join(format!("{name}.png")))
            .unwrap()
    };
    match phase.0 {
        0 => {
            ...
            for role in [
                rvn_ui::PageRole::Title,
                rvn_ui::PageRole::Pause,
                rvn_ui::PageRole::Save,
                rvn_ui::PageRole::Load,
                rvn_ui::PageRole::Settings,
                rvn_ui::PageRole::Gallery,
                rvn_ui::PageRole::History,
                rvn_ui::PageRole::Confirm,
                rvn_ui::PageRole::Dialogue,
                rvn_ui::PageRole::Choices,
                rvn_ui::PageRole::QuickActions,
            ] {
                assert!(
                    document.pages.iter().any(|p| p.role == Some(role)),
                    "Missing required page {role:?}"
                );
            }
            phase.3 = std::fs::read(paths.root.join("menus.rvnui")).unwrap();
            phase.2 = engine.0.state.pc;
            *initial_window = format!(
            ...
            for (slot, title) in [
                (1, "Exemple — Première sauvegarde"),
                (
                    2,
                    "Exemple — Un titre assez long pour vérifier les accents et la disposition",
                ),
            ] {
                engine
                    .0
                ...
                assert_eq!(
                    manager.load(slot).unwrap().pc,
                    engine.0.state.current_interactive_pc
                );
            }
            capture("title");
        }
        ...
        3 => {
            ...
            save.open(SaveMenuMode::Save, SaveMenuOrigin::TitleScreen);
        }
        4 => {
            assert_eq!(engine.0.state.pc, phase.2);
            let (_, text, parent) = texts
                .iter()
                .find(|(text, _, _)| {
                    text.sections
                        .iter()
                        .any(|s| s.value.contains("Un titre assez long"))
                })
                .expect("Filled save summary missing");
            assert!(
                nodes.get(parent.get()).unwrap().size().y + 1.0 >= text.size().y,
                "The long save summary is clipped by its field"
            );
            capture("save");
        }
        5 => {
            save.active = false;
            next.set(VnState::Gallery);
        }
        6 => {
            assert_eq!(engine.0.state.pc, phase.2);
            capture("gallery");
        ...
        7 => {
            ...
            for i in 1..=12 {
                history.add("Personnage de démonstration".into(),format!("Ligne {i} — Une phrase assez longue pour contrôler les accents, les retours à la ligne et le défilement dans chacun des modèles personnalisés."));
            }
        ...
        13 => {
            assert!(dimensions.resizable);
            dimensions.resolution.set(960.0, 600.0);
        }
        14 => {
            if dimensions.width() != 960.0 || dimensions.height() != 600.0 {
                return;
            }
            assert_eq!(engine.0.state.pc, phase.2);
            capture("pause-small");
        ...
        16 => {
            if dimensions.width() != 1280.0 || dimensions.height() != 720.0 {
                return;
            }
            assert_eq!(engine.0.state.pc, phase.2);
            capture("pause-restored");
        }
        _ => {
            assert_eq!(
                std::fs::read(paths.root.join("menus.rvnui")).unwrap(),
                phase.3
            );
